...
module.exports = (acapi, options, cb) => {
  ...
  async.eachSeries(acapi.config.elasticSearch.indices, (index, itDone) => {
    ...
    let url = (acapi.config.localElasticSearch ? 'http' : _.get(server, 'protocol', 'https')) + '://' + _.get(server, 'host') + ':' + (acapi.config.localElasticSearch ? 9200 : _.get(server, 'port'))
    let esConfig = {
      node: {
        url: new URL(url),
        ssl: {
    ...
    async.series({
      ...
      if (acapi.config.environment !== 'test' || !index.resetForTest) return itDone()
